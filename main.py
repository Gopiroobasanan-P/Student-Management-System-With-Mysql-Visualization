...
DB_CONFIG = {
    ...
    "password": "Bala@2002",  # your MySQL password
...
def fetch_data():
    ...
    df = pd.DataFrame(rows, columns=["ID", "Stu_Name", "Course", "Fees", "Join_date"])
    return df

def insert_data(name, course, fees, join_date):
    cursor.execute(
        f"INSERT INTO {TABLE_NAME} (Stu_Name, Course, Fees, Join_date) VALUES (%s, %s, %s, %s)",
...
def update_data(row_id, name, course, fees, join_date):
    cursor.execute(
        f"UPDATE {TABLE_NAME} SET Stu_Name=%s, Course=%s, Fees=%s, Join_date=%s WHERE ID=%s",
...
tree = ttk.Treeview(tab1, columns=["ID", "Stu_Name", "Course", "Fees", "Join_date"], show='headings')
for col in ["ID", "Stu_Name", "Course", "Fees", "Join_date"]:
...
tk.Label(form, text="Join_Date (YYYY-MM-DD)", fg="white", bg="#2c2c2c").grid(row=3, column=0)
...
def search_student():
    ...
    df = fetch_data()
    filtered = df[df['Stu_Name'].str.lower().str.contains(query) | df['Course'].str.lower().str.contains(query)]
...
def generate_pdf():
    ...
    for i, row in df.iterrows():
        c.drawString(50, y, f"{row['Stu_Name']} - {row['Course']} - Rs.{row['Fees']} - {row['Join_date']}")
        y -= 15
        if y < 50:
            c.showPage()
            y = 750
...
def plot_charts():
    ...
    monthly = df.groupby(df['Join_date'].dt.to_period('M')).size()
